...
// mexcuda mex_blochsim.cu mexsimulator.cu magnetization.cu event_manager.cu -dynamic
    ...
    //Get device number from user. Error check for total number of available
    // devices.
    ...
    double* d_myout;
    double* d_mzout; 
    // (void **) casts the input to a ptr to a void-type ptr
    // using void makes it generic
    // d_magn is a ptr, using &d_magn is a ptr to a magnetization-type ptr
    ...
    mexsimulator<<<floor((nelements + threadPerBlock-1)/threadPerBlock),threadPerBlock>>>(d_magn,d_mxout,d_myout,d_mzout,d_nelements,d_numCols,
